...
export async function POST(request) {
  ...
  if (!process.env.ADMIN_EMAIL || !process.env.ADMIN_PASSWORD) {
    return applyAdminCors(
      request,
      failure("CONFIGURATION_ERROR", "Admin credentials are not configured", 503)
    );
  }

  if (email !== process.env.ADMIN_EMAIL || password !== process.env.ADMIN_PASSWORD) {
    return applyAdminCors(request, failure("INVALID_CREDENTIALS", "Invalid credentials", 401));
  }

  try {
    const token = signAdminSession();
    const response = success({ admin: { email }, token });
