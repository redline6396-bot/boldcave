...
function normalizeAudienceTags(tags, fallbackCategory) {
  const rawTags = Array.isArray(tags) ? tags : [fallbackCategory];
  const normalized = rawTags
    .map((tag) => cleanString(tag, 30))
    .filter(validateCategory);

  return Array.from(new Set(normalized.length ? normalized : [fallbackCategory]));
}

function buildProductPayload(body) {
  const category = cleanString(body.category, 30);
  if (!validateCategory(category)) {
    return { error: "Category must be Men, Women, or Unisex" };
  }
  ...
  return {
    payload: {
      ...
      slug,
      category,
      audienceTags: normalizeAudienceTags(body.audienceTags, category),
